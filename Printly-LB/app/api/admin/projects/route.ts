...
export async function POST(request: Request) {
  ...
  await (env as unknown as { BUCKET: R2Bucket }).BUCKET.put(key, file.stream(), { httpMetadata: { contentType: file.type } });
  await sql.query(`INSERT INTO projects (title, slug, category, description, image_url, image_key, sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7)`, [title, unique, category, description, `/api/images/${encodeURIComponent(key)}`, key, Number(rows[0]?.next || 1)]);
...
export async function PATCH(request: Request) {
  ...
  if(body.duplicate){await sql.query(`INSERT INTO projects(title,slug,category,description,image_url,image_key,sort_order,featured,visible,draft,filter_category,customer_goal,solution,materials,finishing,final_result) SELECT title||' — Copy',slug||'-copy-'||extract(epoch from now())::bigint,category,description,image_url,NULL,(SELECT COALESCE(MAX(sort_order),0)+1 FROM projects),false,false,true,filter_category,customer_goal,solution,materials,finishing,final_result FROM projects WHERE id=$1`,[body.id]);}
    ...
    if (neighbor) await sql.transaction(async tx => { await tx.query(`UPDATE projects SET sort_order=$1, updated_at=NOW() WHERE id=$2`, [neighbor.sort_order, current.id]); await tx.query(`UPDATE projects SET sort_order=$1, updated_at=NOW() WHERE id=$2`, [current.sort_order, neighbor.id]); });
  } else {
...
export async function DELETE(request: Request) {
  ...
  if (rows[0]?.image_key) { const { env } = await import("cloudflare:workers"); await (env as unknown as { BUCKET: R2Bucket }).BUCKET.delete(rows[0].image_key); }
  return NextResponse.json({ ok: true });
}
